...
//import org.apache.spark.sql.GeneratorNestedColumnAliasing.canPruneGenerator
...
object NestedColumnAliasing {
  ...
  /**
    ...
    */
  def getNewProjectList(projectList: Seq[NamedExpression],
...
//      if(a.child.isInstanceOf[AttributeReference] && a.child.asInstanceOf[AttributeReference].name.startsWith("_gen_alias"))
//        a.children.flatMap(collectRootReferenceAndExtractValue)
//      else if(a.child.isInstanceOf[GetStructField] && a.child.asInstanceOf[GetStructField].child.isInstanceOf[AttributeReference] && a.name.equals("balances_id"))
//        Seq(AttributeReference(name, StringType, true)(a.exprId, a.qualifier))
//      else
//        a.children.flatMap(collectRootReferenceAndExtractValue)
  ...
  : Option[(Map[ExtractValue, Alias], Map[ExprId, Seq[Alias]])] = {
    ...
    val a1 = exprList.flatMap(collectRootReferenceAndExtractValue)
    val a2 = a1.partition {
      case _: ExtractValue =>
        true
      case _ =>
    ...
    val aliasSub = nestedFieldReferences.asInstanceOf[Seq[ExtractValue]]
      .filter(as => ! as.references.subsetOf(exclusiveAttrSet))
      .groupBy(as => as.references.head)
...
object GeneratorNestedColumnAliasing {
  def unapply(plan: LogicalPlan): Option[LogicalPlan] = plan match {
    ...
    case g: Generate if SQLConf.get.nestedSchemaPruningEnabled &&
      canPruneGenerator(g.generator) => // projectList ++ g.generator.children
      val p2 = NestedColumnAliasing.getAliasSubMap(g.generator.children).map {
        case (nestedFieldToAlias, attrToAliases) =>
