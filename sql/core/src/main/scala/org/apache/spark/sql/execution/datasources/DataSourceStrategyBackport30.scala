...
import org.apache.spark.sql.execution.{RowDataSourceScanExec, SparkPlan, SparkStrategy}
 ...
 */
case class DataSourceAnalysisBackport30(conf: SQLConf) extends Rule[LogicalPlan] with CastSupport {
 ...
 * data source.
 */
class FindDataSourceTableBackport30(sparkSession: SparkSession) extends Rule[LogicalPlan] {
 ...
 */
case class DataSourceStrategyBackport30(conf: SQLConf) extends Strategy with Logging with CastSupport {
...
object DataSourceStrategyBackport30 {
  /**
  ...
  private def translateLeafNodeFilter(predicate: Expression): Option[Filter] = predicate match {
    case expressions.EqualTo(PushableColumnBackport30(name), Literal(v, t)) =>
      Some(sources.EqualTo(name, convertToScala(v, t)))
    case expressions.EqualTo(Literal(v, t), PushableColumnBackport30(name)) =>
      Some(sources.EqualTo(name, convertToScala(v, t)))

    case expressions.EqualNullSafe(PushableColumnBackport30(name), Literal(v, t)) =>
      Some(sources.EqualNullSafe(name, convertToScala(v, t)))
    case expressions.EqualNullSafe(Literal(v, t), PushableColumnBackport30(name)) =>
      Some(sources.EqualNullSafe(name, convertToScala(v, t)))

    case expressions.GreaterThan(PushableColumnBackport30(name), Literal(v, t)) =>
      Some(sources.GreaterThan(name, convertToScala(v, t)))
    case expressions.GreaterThan(Literal(v, t), PushableColumnBackport30(name)) =>
      Some(sources.LessThan(name, convertToScala(v, t)))

    case expressions.LessThan(PushableColumnBackport30(name), Literal(v, t)) =>
      Some(sources.LessThan(name, convertToScala(v, t)))
    case expressions.LessThan(Literal(v, t), PushableColumnBackport30(name)) =>
      Some(sources.GreaterThan(name, convertToScala(v, t)))

    case expressions.GreaterThanOrEqual(PushableColumnBackport30(name), Literal(v, t)) =>
      Some(sources.GreaterThanOrEqual(name, convertToScala(v, t)))
    case expressions.GreaterThanOrEqual(Literal(v, t), PushableColumnBackport30(name)) =>
      Some(sources.LessThanOrEqual(name, convertToScala(v, t)))

    case expressions.LessThanOrEqual(PushableColumnBackport30(name), Literal(v, t)) =>
      Some(sources.LessThanOrEqual(name, convertToScala(v, t)))
    case expressions.LessThanOrEqual(Literal(v, t), PushableColumnBackport30(name)) =>
      Some(sources.GreaterThanOrEqual(name, convertToScala(v, t)))

    case expressions.InSet(e @ PushableColumnBackport30(name), set) =>
    ...
    // down.
    case expressions.In(e @ PushableColumnBackport30(name), list) if list.forall(_.isInstanceOf[Literal]) =>
      val hSet = list.map(_.eval(EmptyRow))
      val toScala = CatalystTypeConverters.createToScalaConverter(e.dataType)
      Some(sources.In(name, hSet.toArray.map(toScala)))

    case expressions.IsNull(PushableColumnBackport30(name)) =>
      Some(sources.IsNull(name))
    case expressions.IsNotNull(PushableColumnBackport30(name)) =>
      Some(sources.IsNotNull(name))
    case expressions.StartsWith(PushableColumnBackport30(name), Literal(v: UTF8String, StringType)) =>
      Some(sources.StringStartsWith(name, v.toString))

    case expressions.EndsWith(PushableColumnBackport30(name), Literal(v: UTF8String, StringType)) =>
      Some(sources.StringEndsWith(name, v.toString))

    case expressions.Contains(PushableColumnBackport30(name), Literal(v: UTF8String, StringType)) =>
...
/**
  ...
  */
object PushableColumnBackport30 {
...
//    def asIdentifier: Identifier = new IdentifierImpl(parts.init.toArray, parts.last)
...
//case class DataSourceStrategyJ1Strategy(spark: SparkSession) extends SparkStrategy {
//  override def apply(plan: LogicalPlan): Seq[SparkPlan] =
//    DataSourceStrategyJ1
//      .apply(spark.sqlContext.conf)
//      .apply(plan)
//}
